...
class TfPoseEstimator:
    ...
    @staticmethod
    def draw_humans_joints(npimg, humans, imgcopy=False):
        if imgcopy:
            npimg = np.copy(npimg)
        actions = []
        human_joints = []
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
                # print(center)
                human_joints.append((BodyPart.get_part_name(body_part)).value)
                info.append([center[0], center[1]])

                # human_joints.append(((BodyPart.get_part_name(body_part)).name, (BodyPart.get_part_name(body_part)).value,center))
                centers[i] = center
                cv2.circle(npimg, center, 3, common.CocoColors[i], thickness=3, lineType=8, shift=0)
            dt = dict(zip(human_joints, info))
            # draw line
            for pair_order, pair in enumerate(common.CocoPairsRender):
                if pair[0] not in human.body_parts.keys() or pair[1] not in human.body_parts.keys():
                    continue
